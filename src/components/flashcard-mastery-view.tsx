...
import { X, Search, Circle, CircleDot, CircleCheckBig, Award, Filter } from 'lucide-react';
...
export const FlashcardMasteryView: React.FC<FlashcardMasteryViewProps> = ({ onClose, selectedSubject }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterLevel, setFilterLevel] = useState<MasteryLevel | 'all'>('all');

  const mockFlashcards: Flashcard[] = [
    { id: '1', front: 'What is photosynthesis?', back: 'Process by which plants convert light into energy', subject: 'Biology', mastery: 'mastered', timesStudied: 12, lastStudied: new Date() },
    { id: '2', front: 'Define momentum', back: 'Mass times velocity (p = mv)', subject: 'Physics', mastery: 'familiar', timesStudied: 7 },
    { id: '3', front: 'Pythagorean theorem', back: 'a² + b² = c²', subject: 'Mathematics', mastery: 'learning', timesStudied: 4 },
    { id: '4', front: 'What is mitosis?', back: 'Cell division process', subject: 'Biology', mastery: 'new', timesStudied: 1 },
    { id: '5', front: "Newton's First Law", back: 'Object in motion stays in motion unless acted upon', subject: 'Physics', mastery: 'mastered', timesStudied: 10 },
    { id: '6', front: 'Derivative of x²', back: '2x', subject: 'Mathematics', mastery: 'familiar', timesStudied: 6 },
    { id: '7', front: 'What is ATP?', back: 'Adenosine triphosphate - energy currency of cells', subject: 'Biology', mastery: 'learning', timesStudied: 3 },
    { id: '8', front: 'Force equation', back: 'F = ma (Force = mass × acceleration)', subject: 'Physics', mastery: 'mastered', timesStudied: 9 },
    { id: '9', front: 'Quadratic formula', back: 'x = (-b ± √(b²-4ac)) / 2a', subject: 'Mathematics', mastery: 'learning', timesStudied: 5 },
    { id: '10', front: 'DNA structure', back: 'Double helix with nucleotide base pairs', subject: 'Biology', mastery: 'familiar', timesStudied: 8 },
  ];
  ...
  const filteredCards = useMemo(() => {
    return mockFlashcards.filter(card => {
      ...
      const matchesSubject = !selectedSubject || selectedSubject === 'All Subjects' || card.subject === selectedSubject;

      return matchesSearch && matchesFilter && matchesSubject;
    });
  }, [searchTerm, filterLevel, selectedSubject, mockFlashcards]);
